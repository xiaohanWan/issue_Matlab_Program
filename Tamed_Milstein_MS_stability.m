% STAB Mean-Square stability test for tamed Milstein
%
...
for k = 1:3
    ...
    for j = 1:N
        ...
        tamed_term = 1 + abs(-lambda*Xtemp - sigma*Xtemp.^5)*Dt;
        Xtemp = Xtemp + (Dt*(-lambda*Xtemp - sigma*Xtemp.^5))./tamed_term + mu*Xtemp.*Winc...
            + Xtemp.*(Winc.^2 - Dt);
        Xms(j) = mean(Xtemp.^2);
    end
